...
**Origen del dominio y por qué la zona entera se administra en Route 53.** La cuenta AWS del equipo está bajo Free Tier, que no incluye el registro de dominios en Route 53 (Route 53 cobra el registro de dominios como servicio aparte, no aplica al Free Tier). El equipo ya tenía un dominio comprado en otro registrador (Hostinger), así que la opción natural era usarlo. El plan inicial fue mínimo invasivo: hacer **subdomain delegation** del subdominio `ticke-t.lumenchat.app` a Route 53, dejando el resto del dominio bajo el DNS del registrador. Esto requiere agregar 4 records tipo `NS` en el registrador apuntando a los nameservers que devuelve Terraform al crear la `aws_route53_zone` hija. Bloqueante: el registrador no soporta records tipo `NS` desde su UI estándar. La alternativa correcta es hacer la delegación al nivel del dominio entero — cambiar los Nameservers del dominio en el panel del registrador por los 4 que entrega `aws_route53_zone` para `lumenchat.app`. Eso es lo que está provisionado hoy, y por eso el módulo `dns` administra la zona completa (incluyendo records del apex como A/AAAA, MX, SPF, etc.) en vez de solo el subdominio.
