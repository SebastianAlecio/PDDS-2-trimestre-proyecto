...
**Límite del sistema.** Dentro de Ticke-T: el formulario web de creación de tickets, el widget de chat embebible, el panel multi-agente, la API que persiste tickets y mensajes, la BD y el almacenamiento de archivos. Fuera del sistema: el portal interno corporativo de cada empresa cliente (Ticke-T solo expone una pieza para embeber en él), el proveedor de identidad/SSO (delegamos el login a un sistema externo — ver § 12), el navegador del usuario final y todos los canales externos de mensajería (WhatsApp, email, redes) que están explícitamente OUT-of-scope en § 11.
...
### 7.2 · Cola de tickets (vista agente)
...
### 7.3 · Detalle del ticket (vista agente, split-view)
...
### 7.5 · Widget de chat (vista colaborador)
...
### 7.6 · Dashboard de métricas (gerente)
...
## 8. Mapeo funcionalidad → componente del curso
...
- **Adjuntos:** `adjuntos[]` — array de objetos puntero al archivo en S3 (ver § 10.3).
...
> **Cerrado en E2:** la pregunta de Base de datos (RDS Postgres vs DynamoDB) quedó resuelta en favor de **DynamoDB single-table**. La justificación completa está en § 10; el resumen ejecutivo del cambio está en el *Resumen de cambios E1 → E2* al inicio.
...
3. **Diseño de los 4 GSIs.** Pasamos los user stories del § 5 a la IA y derivamos los access patterns en orden: US "mis tickets" → GSI1 por colaborador; cola del agente → GSI2 por agente asignado; reporte del gerente → GSI3 con PK constante (`"TICKETS"` y `"MENSAJES"`); filtro estado + prioridad para el panel → GSI4 con SK compuesta. La IA aportó dos refinamientos: hacer `GSI2` **sparse** (solo se popula al asignar agente, no en la creación) para que el índice quede chico, y usar **projection INCLUDE** en `GSI3` con solo `titulo` y `estado` para abaratar el listado del gerente.

4. **Decisión de caché.** Le pedimos a la IA evaluar si valía la pena meter DAX o ElastiCache en el MVP. Concluimos juntos que no: Lambda + DynamoDB on-demand ya entregan latencia single-digit ms para los access patterns dominantes, y el panel se refresca por WebSocket push (no por polling repetido que sería el caso clásico donde la caché ayuda). Quedaron documentadas como evolución futura las dos opciones concretas — DAX si crece el dashboard de métricas, ElastiCache si aparece estado efímero del chat (presence/typing).

### Qué no aceptamos en E2

- **Renombrar los GSIs en código a nombres semánticos** (`MyTicketsByUser-Index`, `AgentQueue-Index`, etc.). La IA sugirió varias rondas hacerlo "para legibilidad", pero implicaba reescribir Terraform + handler. Optamos por mantener `GSI1`..`GSI4` en código y describir el propósito en la prosa del documento (§ 10.2). Trazabilidad sin churn.
- **Cambiar a multi-tenant en el MVP.** La IA insistió varias veces que los `PK` deberían prefijarse con `TENANT#` "por las dudas". Lo rechazamos: el código actual no lo necesita, el cliente target del MVP es mono-tenant por deploy y los 4 GSIs tendrían que reescribirse. Quedó documentado el trade-off en § 10.1.
